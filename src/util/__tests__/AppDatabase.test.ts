...
describe('AppDatabase (with mocked Dexie)', () => {
  ...
  it('naturalKillerCell preloads as SQUARE/SQUARE (Tony 18 Jul: square on both, 200kHz)', async () => {
    const db = new AppDatabase();
    await db.preloadDefaults();

    const nkc = await db.loadData('naturalKillerCell');
    expect(nkc).toBeTruthy();
    expect(nkc.channel1wavetype).toBe('SQUARE');
    expect(nkc.channel2wavetype).toBe('SQUARE');
    expect(nkc.startFrequency).toBe(0);
    expect(nkc.data[0].frequency).toBe(200000);
  });

  it('preload upsert flips an existing SINE naturalKillerCell row to SQUARE (existing installs)', async () => {
    const db = new AppDatabase();
    // Simulate an install that already stored the interim SINE config; the
